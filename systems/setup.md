...
[^ollama]: **ollama**
    ...
    Note that `ollama serve` command procedes `ollama list` for available models, and the `--help` option will list environment variables such as OLLAMA_HOST (default 127.0.0.1:11434). A way to handle .gguf format is as follows,
