...
linking internal projects/personal space (additional requests need to be made). A large collection of R packages (1,449 as of 23/5/2024)
is linked with the latest R distribution, 4.4.0; there are also 15 packages under 4.4.0-gcc11 as well as 20 packages under 4.4.0-icelake. The small number of add-on packages have problems either at the compiling stage (because of higher version of gcc is required) or loading (i.e., compiled under 4.4.0 for cclake but cannot be used under icelake).
