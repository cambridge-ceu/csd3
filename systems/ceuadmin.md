...
linking internal projects/personal space (additional requests need to be made). A large collection of R packages (1,708 as of 1/12/2024, esp. with availability of major machine learning packages)
is linked with the latest R distribution, 4.4.2; there are also 3 packages under R-gcc11. Note that there are limitations with CSD3 so that `sf`, `terra` cannot be updated due to incomplete build of `gdal`/`proj`.
