...
```
However, it seems preferable to use via `virtualenv`; the certification is achieved via `gcloud auto login --no-launch-browser; gcloud config set PROJECT_ID` (e.g. [301.0.0](https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-sdk-301.0.0-linux-x86_64.tar.gz)) followed by `gsutil config`.
