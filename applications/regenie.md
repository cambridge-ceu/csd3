...
This is the first attempt on icelake,

```bash
wget -qO- https://github.com/rgcgithub/regenie/archive/refs/tags/v4.1.tar.gz | \
tar xfz -
cd regenie-4.1/
...
Usage:
 ...
 Main options:
  ...
  -p, --phenoFile FILE          phenotype file (header required starting
      ...
      --eventColList STRING,..,STRING
                                comma separated list of event status names
                                to keep (can use parameter expansion {i:j})
  ...
  -b, --bsize INT               size of genotype blocks
      ...
      --htp STRING              output association files in step 2 in HTP
                                format specifying the cohort name)
