    ...
    if (id == "0") {
      ...
      if (drawingCanvas[id].width > drawingCanvas[id].height * aspectRatio) {
        drawingCanvas[id].xOffset =
          (drawingCanvas[id].width - drawingCanvas[id].height * aspectRatio) /
          2;
      } else if (
        ...
        drawingCanvas[id].yOffset =
          (drawingCanvas[id].height - drawingCanvas[id].width / aspectRatio) /
          2;
      }
        ...
        if (
          ...
          drawingCanvas[id].xOffset =
            (drawingCanvas[id].width -
              data[id].width * drawingCanvas[id].scale) /
            2;
          drawingCanvas[id].yOffset =
            (drawingCanvas[id].height -
              data[id].height * drawingCanvas[id].scale) /
            2;
        }
        if (config.readOnly) {
  ...
  function addPageNumbers() {
    ...
    for (var i = 0; i < slides.length; i++) {
      ...
      for (var j = 0; j < fragments.length; j++) {
        ...
        if (
          Number(fragments[j].getAttribute("data-fragment-index")) + 1 >
          count
        ) {
    ...
    for (var j = 0; j < slideData.events.length; j++) {
      switch (slideData.events[j].type) {
        ...
        case "selectboard":
          ...
          break;
        case "clear":
          getCanvas(template, drawings, board)
            .getContext("2d")
